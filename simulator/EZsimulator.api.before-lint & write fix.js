...
function DWclass()
{
	...
	this.init = function init()
	{
		...
		{				
			...
			if (EZsetup.group != undefined) 
				EZsetup(EZsetup.group);
	...
	this.getPreferenceString = function getPreferenceString(group, name, defaultValue) 
	{
		...
		var value = DWajax.httpGetPref('DWpref', {pref_fn:'get', pref_mode:'string', pref_group:group, pref_key:name, pref_defaultValue:defaultValue})
	...
	this.getDocumentUrl = function getDocumentUrl(url)
	{
	...
	function selection(type)				
	{
		...
		if (dw.selectionCount == undefined || dw.selectionCount > 9) dw.selectionCount = 0;
		for (var count = dw.selectionCount; count<=10; count++)	
		{
			if (count > 9)					//up to 9 selections -- then select all
			{
				var msg = 'All source selected -- '
...
function DWfileClass()
{
	...
	{
		filename = EZurlToFile(filename);	//deletes files of folders
	...
	{					//returns: true, false or null if ajax error
		fileURL = EZurlToFile(fileURL);
		copyURL = EZurlToFile(copyURL);
...
function AjaxClass()
{
	...
	this.httpPost = function httpPost(api, fields, callback)
	{
		...
		for (fieldName in fields)
		{
			...
			text = fields[fieldName].replace(/%/g, escape('%')).replace(/&/g, escape('&')).replace(/\+/g, '@@@')
	...
	this.httpResponse = function httpResponse(xmlhttp, url)
	{
		...
		switch (xmlhttp.status || 999)
		{
			case 200: 	
			{
				if (url.toLowerCase().right(4) == '.xml')
				{
		...
		dw.displayMessage(results[2] || 'ajax return: ' + url);
