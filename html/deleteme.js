	...
	var el = (this instanceof Element) ? this
		...
		onClickEvent = function(action, what)
		{
			...
			if (action == 'click')
			{
				...
				return EZ.event.cancel(event, true);	//event is closure variable
			}
			suggTestOptions.call(this, action, what, event);
		},
		//____________________________________________________________________________
		...
		defaultTags = function(data)
		{
			...
			var tags = {							//slice() dumps EZ()... functions
				...
				/*
				formatPopupUseSugg: {
					arg: EZ('useSugg', 'argFormatSettings'),
					actual: EZ('useSugg', 'actualFormatSettings'),
					expected: EZ('useSugg', 'expectedFormatSettings'),
					note: EZ('useSugg', 'noteFormatSettings')
				},
				formatPopupIcon: {
					arg: EZ('useSugg', 'argFormatSettings'),
					actual: EZ('useSugg', 'actualFormatSettings'),
...
formatWraps: [
	...
	try
	{
		if (evt && evt.type == 'click'			//NEEDS RE-THINKING
		&& el && el.type && !'radio checkbox'.includes(el.type))
		{
			//return;			//button
		}
		...
		switch (action || '')
		{
			...
			{
				/*
				if (el.src && el.src.includes('close.')
				|| el.className && el.className.includes('popup'))
					return true;
	
				if (data && el && data.tags.all.includes(el))
					void(0);
				//else
				//	return EZ.event.cancel(evt,true);
				*/
				break;
			}
			...
			case 'changeIgnored': 				//called by ignoredList onChange()
			{
				...
				var field = data.tags.argsList.EZfield;
	...
	**/
	function __getSuggValues(what)
	{
		...
		if (EZ.isEmpty(values))
		{
			...
			{
				values[id] = suggRules[what][id] || data.defaultValues.all[id];
				//if (id == 'validation')
			...
			switch (g)
			{
				...
				case 'basic': 	
				{
					...
					|| values.okResultsNotSpecified || values.okCtxNotSpecified || values.okArgsNotSpecified))
					{
						...
						EZ('validationLimited').checked = false;
	...
	**/
	function __setBorders(what)
	{
		...
		var note, tag,
			updateRules = what.includes('basic'), 
			updateFormat = data.formatGroups.some(function(g) { return what.includes(g) }),
						
			_setSuggValueTag = function(tag, isDiff, suggValue)
			{									//update suggValue if found on next tag
				...
				else								//must not remove until transition completes
				{
		...
		what.forEach(function(g)
		{
			var gType = (data.okRuleGroups.includes(g)) ? 'rules' : 'format';
			data.diffKeys[g] = [];
			data.diffValues[g] = {};				//not sure if used yet
			
			if (data.suggCount[gType])						
			{
				...
				{									//...id is name for radio
					...
					var isDiff = !isSuggValue(nowValue, suggValue, id)
					if (isDiff
					...
					switch (field.tagType)
					{
						...
						case 'password': 	
						{
							EZ.addClass(el,'sugg', isDiff);
							
							var nextTag = el.nextElementSibling || {className:''};
							if (isDiff)
							{
								suggValue = suggValue || '-blank-';
																	//other > argElementAttr ??
								if (nextTag.className.includes('helpBox'))	
									nextTag.innerHTML = suggValue;
								
								else if (nextTag.className.includes('suggValue'))
								{
									nextTag.setAttribute('data-sugg', suggValue);
									if (nextTag.tagName == 'DIV')	//old school
										EZ.set(EZ('span', nextTag), 'sugg: ' + suggValue);
								}
							}
							//else if (nextTag.className.includes('suggValue'))	
							//	nextTag.removeAttribute('data-sugg');
						...
						{							
							...
							if (el == data.tags.argsList)
								EZ.addClass(['okArgsIgnoredPlus'],'sugg', isDiff);
							break;
						}	
					}
					
														//----------------------------------\\
					if (id in data.tags.tooltip)		//----- set tooltip value/class -----\\
					{									//------------------------------------\\
						tag = data.tags.tooltip[id];
						EZ.set(tag, nowValue);
						EZ.addClass(tag, 'sugg', isDiff);
						_setSuggValueTag(tag, isDiff, suggValue);		
					}
				}
			}
			if (gType == 'rules') 						//addition tooltip processing
			{
				__updateTooltip(g);			
			}
			else										//update tooltip format colunm counts
			{											
				var count = data.diffKeys[g].length;	
				var id = g + 'TooltipFormatHead';		//e.g. argTooltipFormatHead
				EZ.set(EZ('diffCount', id), count.wrap('()'))
				EZ.set(EZ('code', id), data.diffKeys[g].join(', '))
				EZ.addClass(id, 'diff', count);
				
				var col = g.toTitleCase();
				EZ.addClass(data.tags.dataHead, 'suggFormatExists' + col, data.suggCount[g]);
				EZ.addClass('dataHead', 'suggFormatDiff' + col, count);
			}
		});	//...end of updating
		...
		if (updateRules)
		{
			...
			note = (!data.suggCount.rules) ? 'No suggested rules'
				 : (diffCount === 0) ? 'using all suggested rules'
				 : EZ.s('# rules', diffCount) + ' -- not suggested';
			
			data.tags.note.innerHTML = note;		//footer note for ok rules popup
			
			var el = data.tags.note.parentElement;
			EZ.set(EZ('div', el), data.diffKeys.rules.join(', '));
			EZ.removeClass(EZ('code', el), 'hidden', diffCount);
					
													//tooltip open link text in note column
			if (data.suggCount.rules && diffCount)
				note = EZ.s('# un-suggested rules', diffCount);
			
			EZ('suggTitle').innerHTML = note;		
			EZ.addClass(EZ.el.parentElement, 'sugg', diffCount);
		
			EZ.addClass('dataHead', 'suggRulesExist', diffCount);
		}
		...
		if (updateFormat)							
		{
			...
			data.formatGroups.forEach(function(what)
			{
				...
				diffCount = keys.length;
				
				var uniqueKeys = keys.remove(data.diffKeys.format);
				data.diffKeys.format = data.diffKeys.format.concat(uniqueKeys);
				
				note = 'no suggested setting';
				if (data.suggCount[what])
				{
					note = (diffCount === 0) ? 'all suggested settings'
						 : EZ.s('# settings', diffCount) + ' -- not suggested';
				}
				else if (diffCount)
					note += ' shared settings';
													//each format column popup note
				EZ(what + 'FormatNote').innerHTML = note;

				var el = EZ.el.parentElement;		//format col popup footer note tooltip
				EZ.set(EZ('div', el), keys.join(', '));				
				EZ.removeClass(EZ('code', el), 'hidden', keys.length);
				

				
													//icon border if any not sugg
				//EZ.addClass(data.tags.formatPopupIcon[what], 'sugg', diffCount);
				//EZ.addClass(data.tags.formatPopupUseSugg[what], 'hidden', !data.suggCount[what]);
				
				__updateTooltip(what);
				EZ.addClass('dataHead', 'haveSuggFormat', diffCount);

			});
			diffCount = data.diffKeys.format.length;
			
			note = (!data.suggCount[what]) ? 'No suggested format settings'
				 : (diffCount === 0) ? 'using all suggested format settings'
				 : EZ.s('# un-suggested format settings', diffCount);
			
			EZ('suggFormatTitle').innerHTML = note;	
		}
		
		var noteTag = EZ('okRulesNote');				//tooltip popup legend / notes
		
		var show = EZ('notSelected', 'suggTooltipCode');
		EZ.addClass('notSelectedNote', 'hidden', show.undefined);
		
		show = show || (EZ('sugg', 'suggTooltipCode').parentElement != noteTag)
		EZ.removeClass(EZ('sugg', noteTag),  'hidden', show);
			
		show = (EZ('notSugg', 'suggTooltipCode').parentElement != noteTag)
		EZ.removeClass(EZ('notSugg', noteTag),  'hidden', show);
	}
	//________________________________________________________________________________________
	/**														
	 *	format and update suggRules tooltip for what group
	**/
		...
		switch (what)
		{
			case 'extended': 	
			{
				...
				html = (validation != 'extended') ? 'NOT selected'
					 : (count === 0) ? 'using <b>all</b> suggessted settings'
					 : ('&nbsp;using ' + count + ' of ' + total 
					   + ' <b>un-suggested</b> settings&nbsp;').wrap('<span class="sugg">');
			...
			case 'other': 	
			{
				...
				html = (count === 0) ? 'using <b>all</b> suggessted settings'
					 : ('&nbsp;using ' + count + ' of ' + total 
					   + ' <b>un-suggested</b> settings&nbsp;').wrap('<span class="sugg">');
	...
	**/
	...
	{											
		...
		tip = tip.replace(/ plus un-listed/i, '+unlisted').replace(/(on fn statement|"|\*)/gi, '');
	...
	**/
	...
	{													
		var _addSuggValueClick = function(tag, el, what)
		{									//add onclick evt handler to update el/what settings
			var nextTag = tag.nextElementSibling;
			if (EZ.getTagType(nextTag) != 'span')
				return;
											//catch click on suggValue tags
			nextTag.onclick = onClickEvent.bind(el,'suggValueClick', what);
		}
		
		for (var what in data.tags.groups)
		{
			...
			{										//EZ.getTagType() only returns 'select'
				...
				switch (tagType)
				{
					case 'select-multiple':			//uses EZ.field.onChange
						break;
					...
					case 'select-one':
					{
						el.onchange = (what in data.tags.sharedFormat) 
									? onChangeEvent.bind(el,'formatChange')
									: onChangeEvent.bind(el,'change');
						
						if (data.formatGroups.includes(what))
							_addSuggValueClick(el, el, what);
						break;
					}
				}	
			});
		}
		data.tags.tooltip = {};							//get tooltip settings tags
		//var fieldValues = EZ.get(data.allGroups, 'suggTooltipCode')
		//fieldValues.tagList.forEach(function(tag)
		...
		[].forEach.call(tags, function(tag)
		{
			...
			var id = results[1];
			var el = EZ(id, null);
			if (!el) return;							//bail if no associated tag found
			
			var tagType = el.type || EZ.getTagType(el)
			switch (tagType)							//add onclick handler
			{
				case 'checkbox': 						//does el.click()
					tag.onclick = onClickEvent.bind(el, 'click');
					break;
				
				default:							//suggTestOption('suggValueClick',what)
				{
					var what = '';
					for (var g in data.tags.groups)
					{
						if (!data.tags.groups[g].includes(el)) continue;
						what = g;
						break;
					}
					if (what)
						_addSuggValueClick(tag, el, what);
				}
			}
