...
EZ.test.message = (function _____EZtest_message_____()
{
	...
	options = EZ.options.call('globalDefault', defaultOptions);
	//==========================================================================================
	var ___ = function EZtest_message(messageName, opt)
	{
		...
		**/
		...
		var critical = 'highlight cirtical';
		...
		switch (messageName)
		{
			...
			//------------------------------------------------------------------------------------
			{
				e = function ___loadScriptScript(){}	
				var file = opt;
			...
			//------------------------------------------------------------------------------------
			{
				e = function ___testScriptBeforeAny(){}	
			...
			//------------------------------------------------------------------------------------
			{
				e = function ___testScriptBefore_firstRun(){}	
				...
				return;							//no message just info;
			}
			//------------------------------------------------------------------------------------
			case 'prescan-done': 	
			//------------------------------------------------------------------------------------
			{
				e = function ___prescan_done(){}	
				if (v.counts.prescan)
	...
	/**
	 *	if msg is null, clear infoText and alerts close [-] -- ignore if blank or undefined
	**/
	___.info = function ___addInfo(msg, className, detail)
	{
		if (msg === null)
			___.infoClear();
		else if (msg)			
		{
			var el = EZ('testInfoText');
			className = _appendMessage.call(el, msg, className, detail);
			//if (className)
			//	EZ.addClass('testInfoTop', 'top');
		}
		return !msg;
	}
	//__________________________________________________________________________________________
	/**
	 *	move focus off summary tag to hide blue border
	**/
	___.infoBlur = function ___infoBlur(evt)
	{
		var el = evt.target;
	 ...
	 *	clear top ifo layer
	**/
	___.infoClear = function ___clear(what)
	{
		if (what != 'alert')
		{
			EZ.removeClass('testInfoTop', 'top');
			EZ.set('testInfoText', '');	
		}
		var el = EZ('testAlertCount');
		if (el.innerHTML)
			EZ.set(el, 'cleared @ ' + EZ.formatTime() );
		EZ.set(el, 0);
		EZ.addClass(['infoAlertWrap', 'infoAlertFlag'], 'hidden');
		EZ.removeClass('infoAlertFlag', 'starburst');
	}
	//__________________________________________________________________________________________
	/**
	 *	
	**/
	___.infoAlert = function ___addAlert(msg, className, detail)
	{						
		if (!msg) 
			return ___.infoClear('alert');
		
		var count = EZ.get('testAlertCount');
		count = EZ.toInt(count);
		EZ.set(EZ.el, ++count);
		
		var el = EZ('testAlertText');
		_appendMessage.call(el, msg, className, detail);
		
		___.infoAlert.message = _appendMessage.message;
		___.infoAlert.detail = _appendMessage.detail;

		EZ.removeClass(['infoAlertWrap', 'infoAlertFlag'], 'hidden');
		EZ.addClass('infoAlertFlag', 'starburst');
	}
	//__________________________________________________________________________________________
	/**
	 *	append detail/summary tag if detail supplied otherwise append <div> tag
	 *	if className supplied, add to either summary or line tag
	**/
	function _appendMessage(msg, className, detail)
	{	
		var el = (this instanceof Element) ? this : this.toString();
		if (detail === undefined && className instanceof Object)
		{
			detail = className;
			className = '';
		}
		...
		_appendMessage.message = msg = (msg += '').trim();
		_appendMessage.detail = '';
		if (!detail)
		{
...
//			className = 'top';
			...
			detail = stackTrace.toString('html');
		}
		else if (detail instanceof Object)
			detail = EZ.format.value(detail, options);
		else
			detail += '';
		
		_appendMessage.detail = detail;		
		var tags = {
			details:
			{
				...
				nodes:
				{
					summary: {
						...
						margin: '0 0 2px -1px',
						whiteSpace: 'nowrap',
						overflow: 'hidden'			//restrain blue border focus right margin
						},
						class: 'top ' + className,
						nodes: {
							span: {
								nodes: [
									{					
										tagName:'span',	//any maxWidth seems to leaves room for next span but
										styles: {		//value ignored (i.e. uses 100% of table-cell width)
											maxWidth: '200px',
											display: 'table-cell',
											marginLeft: '10px',
											overflow: 'hidden',
		...
		g.detail = EZ.createTag(tags, el);
		return className;
	}
	//==================================================================================================
	return ___;								//no options, properties or prototype functions
})();


//___________________________________________________________________________________________
EZ.test.message.test = function()
{	
	...
	*/
	...
	// . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
	//var msg = EZ.test.message;
	//EZ.test.run(msg, 'test message')
	msg = 'now is the time for all good men to raise to the challenge'
	obj = {a:0, b:'msg, arr, ctx, arg, args, o, obj, note, ex, exfn, notefn, fn, val, rtnValue'}
	EZ.test.run(EZ.test.message, EZ.test.message.info, 'highlight className:true', true, obj)
	EZ.test.run(EZ.test.message, EZ.test.message.info, 'no className', obj)
	EZ.test.run.call(EZ.test.message, EZ.test.message.info, 'attention ' + msg, 'attention', 'houston')

	//_______________________________________________________________________________________
	EZ.test.run.call(EZ.test.message.info, 'attention ' + msg, 'attention', 'houston')
	
	//EZ.test.run(EZ.test.message, EZ.test.message.info, 'attention ' + msg, 'attention', 'houston')
...
EZ.test.variants = function ___EZtest_variants___(fnScript, testScript)
{
	...
	**/
	...
	this.initScroll = function initScroll(testKey)
	{
		...
		EZ('fnStatementCol').style.display = 'none';	//hide to analyize scroll columns
	...
	//	EZ('fnStatementCol').style.width = fnStatement.style.width = '';
		...
		var height = EZ.get('maxHeightTestRowsShared') + 'px';
	...
	//		var testno = tr.EZ('testno').innerHTML;
		...
		rows.forEach(function(tr)						//init scroll td/div column css
		{
			...
			scroll.classNames.forEach(function(className)
			{
				...
				div.EZ(['details'], null).forEach(function(el)
				{									//open details for scroll calculation
	...
	//		var testrun = EZ.test.data.testrun[testno - 1];
			...
			scroll.classNames.forEach(function(className)	//for all scroll columns...
			{
				...
				EZ.toArray(expanded).forEach(function(scrollClass)		
				{											//for all prior expanded columns...
	...
	this.updateScrollBars = function updateScrollBars(isFilterChange)
	{
		...
		for (var pass=1; pass<=3; pass++)
		{
			...
			//	var testno = EZ.toInt( EZ.get(EZ('testno',tr)) );
				...
				//		scroll.autoWidths[className] = autoWidth;	//redundant but so what
						...
						div.EZ(['details'], null).forEach(function(el)
						{									
							el.open = '';					//close details after scroll determined
						});
		...
		EZ('fnStatementCol').style.width = argsWidth;
