...
/*---------------------------------------------------------------------------------------------
Removes html tags -- cloned from: EZregex.js::formatSpecial()
---------------------------------------------------------------------------------------------*/
function EZstripHtmlTags(html, tags /* TODO */)
{
	var openTagPattern = "<(\\w+\\s*)([^>]*?)(/?>)";
	var innerTagPattern = "([\\s\\S]*?)"
	var closeTagPattern = "<(/\\1\\s*>)"
	var fullTagRegExp = RegExp(openTagPattern+innerTagPattern+closeTagPattern, 'g');

	html = html.replace(fullTagRegExp, '$4')
	html = html.replace(RegExp(openTagPattern, 'g'), '');	//singular tags
...
function EZlog(heading,textIn,options)
{
	...
	//if (value == EZ.undefined) return;	//something amuck
