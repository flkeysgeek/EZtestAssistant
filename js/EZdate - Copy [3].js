...
EZ.options.date = {
	...
	baseFormats: //base formats must be set in following order:
			   ...
			   + ' yyyy year yr dd da mo daySuffix day mm m d'
	...
	{					//dates -- all formats blank if no date component (i.e. date <= 1/1/1970)			
		...
		da:				"dayName.substr(0,3)",			//3 char day of week e.g. "Sun"
		day:			"dayName",						//full day of Week e.g. "Sunday"
		daySuffix:		"d + daySuffix",				//day of month with suffix e.g. 1st, 2nd 9th
...
var val = format			
			...
			{
				format = format.replace(key, formats[key].replace(/(.)/g, '$1--'));
				if (val != format) 
					void(0);
				val = format;
			})
		...
		formatList.remove(Object.keys(formatList)).forEach(function(fmt)
		{												//set custom formats
			var format = dateFormats[fmt];
			var keys = Object.keys(formats).sort(sortByLength);
			keys.forEach(function(key)
			{
				var regex = key.replace(RegExp('([*])', 'g'), '\\$1');
				format = format.replace(regex, formats[key].replace(/(.)/g, '$1--'))			
			});
			formats[fmt] = format;
		});
