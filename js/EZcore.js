/*---------------------------------------------------------------------------------------------
***** used by some DW simulator jsp's *****
...
*/
/*---------------------------------------------------------------------------------------------
EZgetEl(selectors, doc)       -- FULL FUNCTIONALITY --
...
UNLESS called from EZgetAll() in which case all selectors are processed. In either case, 
...
TODO: 
	...
	var stopAfterFirst = /^(RZ|EZ)_$/.test(callerName)		//always stop if caller EZ_()
					  ...
					  || (!EZis(els) && els.length > 1);	//...multiple selectors but not els
	
--------------------------------------------------------------------------------------------------*/
function EZgetEl(selectors, nodes)
{
	var settings = EZis(nodes, EZgetEl.getSettings) 
				 ? EZ.merge(nodes, {selectors:selectors})		//called with pre-defined settings
				 : new EZgetEl.getSettings(this, arguments); 	//breakpoint for settings not defined
	...
	while (settings.selectors.length > 0 && settings.roots.length > 0)	
	{
		...
		//---------------------------------------------------------------------------------- 
		if (EZisEl(settings.selectors))	//nodes exploded && (!settings.nodes.length || EZisEl(settings.nodes)))
		...
		{											
			...
			{											
				...
				{										
					//-----------------------------------------------------------------------------
					// recursively call EZgetEl() for chained selections from:
					...
					//-----------------------------------------------------------------------------
					var chainedSelections = EZtoArray(sel,' ');	
	...
	if (EZ.quit) return false;
						...
						nodes.forEach(function forEachChainedSelector(sel)	
						{									//get and save ALL matching tags  . . .	
							EZgetEl_addElements(nodes, EZgetEl(node, settings.baseSetting));
						});									//last selector applied to all matches
					});										
					
					//----- if selector is element, array of elements or html collection
					if (EZisEl(sel)							//...only add elements descendant of roots??
	...
	if (EZ.quit) return false;
					...
					return parts.every(function forEverySelectorPart(sel)
					{
						...
						if ('#'.indexOf(prefix) < 1)
						{										//try for id match
							e = EZgetEl.getDocument(root).getElementById(sel);		
							if (EZisEl(e) && EZgetEl_getDescendants(e,root) != null) 
							{ 	
								if (EZgetEl_addElements(e)) 	//quit if one is enough
									return false;
							}
	...
	if (settings.one && tags != null)		//if returning 1st tag...
		return EZbindElements(tags[0]) 			//...bind 1st tag if not yet bound
	else									//if returning Array or html collection...
		return EZbindElements(tags)				//...bind Array/collection and tags
	//================================================================================
	/**
	...
	function EZgetEl_addElements(nodes)
	{
		...
		if (settings.asis && !tags.length && EZisArrayLike(nodes))
			tags = nodes;		//save html collection as is if no other tags
		
		else
		{
			if (!EZisArrayLike(tags))			//html collection...
				tags = [].slice.call(tags);		//...copy to new Array
			
			EZtoArray(nodes).forEach(function(el) 
			{ 									
	 ...
	 *	of element that called EZgetEl() -or- all elements if EZgetEl() was not called from element.
	 *
	...
	{
		var tags = [];
		[].every.call(EZgetEl.getDocument(root).forms, function forEveryForm(form)
		{
	...
	function EZgetEl_isSelectorType(sel)
	{
		...
		if (typeof(sel) == 'object' && !EZisEl(sel))
			return false;
 ...
 */
EZgetEl.getDocument = function EZgetEl_getDocument(el)
{
	return EZis(el, document.constructor) 
...
-----------------------------------------------------------------------------------*/
EZgetEl.getSettings = function EZgetEl_getSettings(ctx, args)
{
	//----- context root element(s) -- EZget/EZset pass context elements as nodes
	this.roots = ctx == window || typeof(ctx) == 'function' ? []
			   : EZtoArray(ctx);			//breakpoint here for element context
	...
	var isMultipleSelectors = (EZisArray(this.selectors) && !EZ.isEl(this.selectors));
	...
	this.nodes = EZtoArray(args.shift());				//clone nodes html collection as...
	...
	this.baseSettings = EZmerge({mergeOptions:{append:false}}, settings, defaultOptions, options);
	EZmerge(this, this.baseSettings);				

	// base settings for recursive calls to EZgetEl()
	this.baseSettings = EZmerge(settings, {legacy:false, notFound:[], all:true, one:false});
	...
	|| (this.roots.length > 1 || !EZis(this.doc,document.constructor)))
		this.legacy = false;
	...
	/**
	 *	recursive calls to EZgetEl() to find elements for non-element selectors
	 *	no nodes filter, array of all selections, empty array if no tags found.
	 *	Uses pre-defined settings so EZgetEl_getSettings() is not called again.
	 */
...
EZ.none = function EZnone(/* [settings | selectors | isUnique] */)
{
	...
	var settings = EZis(args[0],EZgetEl.settings) ? args.shift() : '';
	var isUnique = settings ? '.uniqueEZnone'.ov(settings) : args.shift();
	var isOops = isUnique !== false;
	
	var selectors = EZtoArray('.selectors'.ov(settings, 'unknown'));
	var item = EZcollapse(selectors);		//1st selector
...
{
	var tags = EZtoArray(el);		//returns empty Array if el null, blank or undefined
	var values = [];
	[].forEach.call(tags,function(el)
	{
		var val = EZgetValue(el, defaultValue);
		values = values.concat( EZtoArray(val, '|') )
	});
	return values;
}
/*---------------------------------------------------------------------------------------------
...
{									//el arg is single element, html collection or selector(s)
	el = EZgetEl(el,true)[0];		//get 1st element of collection or matching 1st selector
	if (!el || el.undefined) 				
...
{
	el = EZgetEl(el,true)[0];		//get 1st element of collection or matching selector
	if (!el || el.undefined) 				
		return '';					//TODO: if no field or element found -- return blank ??
		...
		{										//get all radio group tags
			...
			{									//length=1 is possible forgot when
				...
				for (var i=0; i<tags.length; i++)
				{
					...
					{
						tags[i].checked = true;
						return EZgetValue(name);		//return if matching value found
					}
...
//TODO: 1st empty innerHTML
				if (el.innerHTML != EZ.undefined)
				{
					...
					return el.innerHTML = value;		//return innerHTML if defined
				}
			}
		}
	}
