...
/*global 
	...
	EZgetPref, EZsetPref, EZsetValue,
	
	EZcommentStripper,
	EZdisplayMessage, EZformatdate,
 ...
 *	
 */
EZ.ls.getGroup = function EZls_getGroup(group, key, defaultValue)
{
	return EZ.ls.group;
}
/**
...
EZ.ls.get = EZ.getLS = function EZgetLS(key, defaultValue)
{
	...
	(keys[0]) ? keys.unshift(EZ.ls.group.trim()) : keys.shift();
...
EZ.ls.set = EZ.setLS = function EZsetLS(key, value)
{
...
EZ.getBrowserOffsets = function EZgetBrowserOffsets()
{
	...
	if (!win.innerWidth)	//IE...
	{
		...
		{					//...strict mode
			browserOffsets.width = win.document.documentElement.clientWidth;
			browserOffsets.height = win.document.documentElement.clientHeight;
			browserOffsets.x = win.document.documentElement.pageXOffset;
			browserOffsets.y = win.document.documentElement.pageYOffset;
		}
		else
		{					//...quirks mode
			browserOffsets.width = win.document.body.clientWidth;
			browserOffsets.height = win.document.body.clientHeight;
			browserOffsets.x = win.document.body.pageXOffset;
			browserOffsets.y = win.document.body.pageYOffset;
		}
	}
	else					//WC3...
	{
		browserOffsets.width = win.innerWidth;
		browserOffsets.height = win.innerHeight;
		browserOffsets.x = win.pageXOffset;
		browserOffsets.y = win.pageYOffset;
	}
...
-----------------------------------------------------------------------------*/
EZ.hide = function EZhide(tags, isTrueFalse)
{
	isTrueFalse = arguments.length < 2 || isTrueFalse;
	if (!isTrueFalse) return EZ.show(tags);
...
/*-----------------------------------------------------------------------------
hide tags -- display=block
-----------------------------------------------------------------------------*/
EZ.show = function EZshow(tags, isTrueFalse)
{
	isTrueFalse = arguments.length < 2 || isTrueFalse;
	if (!isTrueFalse) return EZ.hide(tags);
	...
	if (el && EZ.getStyle(el,'display') == 'none')
		el.style.display = 'block';
	if (isTrueFalse && tags[0] && tags[0].scrollIntoViewIfNeeded)
...
{	
	...
	var status = EZ.toArray(el, ', ').remove().some(function(el)
	{
		...
		var status = classNames.some(function(className)
		{
			...
			{
				var is = classArray.length > 0 && classArray.includes(className);
				if (action == 'has') 
					return is;							//quits some loop if true
					
				else if (action == 'add' && !is)		
					classArray.push(className);
			
				else if (action == 'remove' && is)		
...
Cloned from EASY.formfields.js but not used or tested.
--------------------------------------------------------------------------------------------------*/
...
{											
	...
	//TODO: does not work -- perhaps document.body.parentNode --> el.parentNode ??
	if (EZ.MSIE || EZ.isIEw3c)			
	{
		//http://marcgrabanski.com/simulating-mouse-click-events-in-javascript/
		var doc = document;		//not used yet
		var args = [eventName,
		...
		evt = eventName.startsWith('mouse') ? createEvent('MouseEvents')
			: eventName.startsWith('key') ? createEvent('KeyBoardEvents')	//??
			: createEvent('Events');										//??
	...
	else		//worked great before cloning -- not tested
	{
		//https://developer.mozilla.org/en-US/docs/Web/Guide/Events/Creating_and_triggering_events
		evt = eventName.startsWith('mouse') ? new MouseEvent(eventName, opts)
			: eventName.startsWith('key') ? new KeyEvent(eventName, opts)	//??
			: new Event(eventName, opts)									//??
	}
	el.dispatchEvent ? el.dispatchEvent(evt) 
					 : el.fireEvent('on' + enentName, evt);
};
// . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
//_____________________________________________________________________________________________
EZ.event.trigger.test = function()
{
	...
	EZ.test.run(arg0, 							{EZ: {ex:function()
												{
												},	note:''	}})
}
/*-----------------------------------------------------------------------------
