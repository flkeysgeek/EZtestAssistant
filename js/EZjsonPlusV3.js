...
void(d,dd);
	 ...
	 */
	EZjsonPlusV3.prototype.escape = function JSON_escape_____(value, options, rtnValue)
	{	
...
if (!isIgnored)
	...
	{
		options = EZ.options.call(this.options.common, options);
		rtnValue = rtnValue || new EZ.returnValue(this,options);
				
		var msg;
		void(msg)
		var jsonUtils = this.getMarkers(obj, options, rtnValue);
		var data = jsonUtils._data;
		var json = data.json
		rtnValue.set('json', json);
//		var json = ___cleanStrings(data.json);
		//json = jsonRaw;
//			if (!json.includes(marker))
		
		var markerMap = data.markerMap;
		...
		//rtnValue.setList('markers', markerMap)

		//markerList.forEach(function(dotName)
		for (var dotName in markerMap)
		{													
			var marker = markerMap[dotName];
			rtnValue.addListItem('markers', marker.script, dotName)
			try
			{	
				if (!marker || marker.type == 'ignore')
					void(0);
														
				else if (marker.type == 'array')		//named Array properties marker
					_unescape_Array(obj, dotName);
														
				else if (marker.type == 'html')			//html Element marker
					_unescape_HTML(obj, marker.script);			
				
				else if (marker.type == 'value')		//all other escape markers -- 
					jsonUtils.evalScript('value', dotName, marker.script);
					
				else
					void(0);
				//return _unescape_Value(obj, dotName, markerScript, outer) ? '' : all;
				void(_unescape_Value);
			}
			catch (e)
			{
				jsonUtils.jsonError(e.message, dotName, marker.script, marker.outer);
			}
		}
if (false)
{
		...
		{											
			do
			{
				...
				json = json.replace(regex, function(all, markerScript, more)
				{										
					var dotName = '$';
					markerScript += more;
					/*
					var script = 'obj=' + markerScript;
					if (more)						//e.g. /xyz/gim;#.lastIndex=1
						script += more.replace(/#/g, 'obj');
					
					//if (!_unescape_marker(dotName, markerScript, all))
					//	return all;
					*/
					jsonUtils.evalScript('value', dotName, markerScript, all)					
					return '';
				});
				if (!json) break;
				...
				json = json.replace(regex, function(all, outer, markerScript, dotName, unusedKey, arrayDotName)
				{													
					dotName = dotName || arrayDotName;
					/*
					if (!_unescape_marker(dotName, markerScript))
						return all;
					*/
					//var type = jsonUtils.type;
					
					try
					{	
						if (!markerMap[dotName] || markerMap[dotName].type == 'ignore')
							void(0);
																
						else if (arrayDotName !== undefined)	//named Array properties marker
							_unescape_Array(obj, arrayDotName);
																//html Element marker
						else if (markerScript.endsWith(options.escapeHTML))
							_unescape_HTML(obj, markerScript);			
						
						else									//all other escape markers -- 
							jsonUtils.evalScript('value', dotName, markerScript, outer);
					...
					{
						jsonUtils.jsonError(e.message, dotName, markerScript, outer);
					}
					return all;
				});
			}
		...
		{
			jsonUtils.jsonError(e.message);
		}
		/*
		json.replace( RegExp(marker + '(.*)', 'g'), function(all, markerScript)
		{
			errorCount++;
			rtnValue.mergeListItem('unescape_unprocessed_markers', marker, markerScript);
		});
		*/
}
		if (data.message.length)
		{
			msg = 'JSON.plus.unescape() [' + EZ.s('# errors', data.message.length) + ']'
				+ ' -- to see json: \n'
				+ 'JSON.plus.unescape.returnValue.getDetails()'
			msg += '\n' + data.message.join('\n');
			rtnValue.addMessage(msg);
			//if (jsonUtils.json)
			//	rtnValue.addDetails( ___linenoFormat(jsonUtils.json) );
		}
		//=======================================
		return rtnValue.save(obj);
		//=======================================
		 ...
		 *
		 */
		function _unescape_HTML(obj, markerScript)	
		{										
			var dotName = markerScript.clip(options.escapeHTML.length+1).split('.');	
			var key = dotName.pop();
			if (!key)
				return false;
		 ...
		 */
		function _unescape_Value(obj, dotName, markerScript, outer)
		{
			/*
			var msg = '';
			var value = markerScript.substr(dotName.length+1);	//value after '='
			value.replace(/(\$(.+?)?\.(.*))/, function(all, dotName, subkey, key)
			{												//if value is $...
				var o = subkey ? subkey.ov(obj) : obj;
				if (o instanceof Object && key in o)	
					return;			
				msg = _unescape_invalid(markerScript, dotName, outer);	//source dotName undefined
			});	
			if (msg) return msg;
			*/	
															//e.g. --> $.1=/xyz/;$.1.lastIndex=3
			var script = markerScript.replace(/;#/g, ';' + dotName);	
															//then change number's -- e.g. $.1 --> $[1]
			script = script.replace(/\.(\d+)(?=([.=]|$))/g, '[$1]');	
			script = script.replace(/\$/g, 'obj');			//and finally change $ --> obj
			
			return jsonUtils.evalScript('value', dotName, script, outer);
		}									
	}
	/*--------------------------------------------------------------------------------------------------
	 ...
	 */
	EZjsonPlusV3.prototype.getMarkers = function JSON_getMarkers___(obj, options, rtnValue)
	{
		options = EZ.options.call(this.options.common, options);
		rtnValue = rtnValue || new EZ.returnValue(this,options);
		
		var jsonUtils = new ___jsonUtils(obj);
		var data = jsonUtils._data;

		//jsonUtils.obj = obj;		
		
		var marker = data.marker = options.escapeMarker.wrap('@');
		var markerMap = data.markerMap = {};	
		
		//var obj = jsonUtils.obj		
		var processed = [];
		if (obj)
			_parseObject(obj, '$');
		//========================================================================================
		else
		{
			...
			{										
				...
				markerMap.$ = {type:'value', script:script, outer:all}
				return '';
			});
			if (!json) return jsonUtils;
			...
			{													
				dotName = dotName || arrayDotName;
				var type = 'na';
				if (!jsonUtils.dotNameExists(dotName))
				{
					type = 'ignore';
					this.invalidMarker(dotName, markerScript, outer);
				}
				else if (arrayDotName !== undefined)				//named Array properties marker
				{												
					type = 'array'
				}		
				else if (markerScript.endsWith(options.escapeHTML))	//html Element marker
				{													
					type = 'html'
				}
				else
				{
					type = 'value';
					var equalCount = markerScript.count('=');		//some marker validation
					var poundCount = markerScript.count('#');
					if ( (equalCount-1) != poundCount )
					{
						type = 'ignore';							//ignore invalid dotName value
						jsonUtils.jsonError('invalid marker syntax (=/#)', markerScript, outer);
					}
					else
					{												//value after '='
						var value = markerScript.substr(dotName.length+1);
						value.replace(/(\$(.+?)?\.(.*))/, function(all, dotName, subkey, key)
						{												
							if (jsonUtils.dotNameExists(subkey, key)) return;
							type = 'ignore';						//ignore invalid dotName value
							this.invalidMarker(dotName, markerScript, outer);
						});	
					}
				}
				markerMap[dotName] = {type:type, script:markerScript, outer:outer}
			});
		}
		//========================================================================================
		if (Object.keys(markerMap).length === 0)
			jsonUtils.markerMap = null;
		//======================
		return jsonUtils;
		//======================
		//________________________________________________________________________________________
		 ...
		 *	
		 */
		function _parseObject(obj, dotName)	
		{	
			var keys = Object.keys(obj)
			var idx = keys.indexOf(marker);
			if (idx != -1)
			{
				void(0);							//TODO: key is marker ??
			}
			keys.forEach(function(p)
			{
				...
				var script = value;
				var outer = value;
				var type = EZ.getType(value, true)
				// . . . . . . . . . . . . . . . . .
				switch (type)
				{
					case 'String': 
					{
						if (value.startsWith(marker)) 
						{
							isMarker = true;
							type = 'value';
							script = value.substr(marker.length + 2);
						}
						break;
					}
					case 'Array': 	
					{
						type = 'array'
						script = value.substr(marker.length + 2);
						markerMap[dotName] = {type:type, script:script, outer:value}
					...
					case 'Object': 	
					{
						if (marker in value)
						{
							...
							dotName += '.' + p;
							script = value[marker];
							type = (script.endsWith(options.escapeHTML)) ? 'html'
																		 : 'value'
							outer = marker.concat(': "', script, '"');
						}
						break;
					}
					default:	return;
				...
				// . . . . . . . . . . . . . . . . .
				if (isMarker === true)
					markerMap[dotName] = {type:type, script:script, outer:outer}
				
				else if (type == 'Object'  && !processed.includes(value))
				{
					processed.push(value)
					_parseObject(value, dotName + '.' + p);
				}
			});
		}
	}
	...
	{	
		...
		void(___cleanStrings, ___hasMarker, ___jsonUtils, ___jsonType, ___isCircular, ___linenoFormat);
	}
	/*----------------------------------------------------------------------------------------------
	internal functions
	----------------------------------------------------------------------------------------------*/
	//______________________________________________________________________________________________
	/**
	 *	used by getMarkers() -- indirectly by unescape() -- parse() may call unescape()
	 */
	function ___jsonUtils(obj)
	{
		...
		try
		{
			data.json = JSON.stringify(obj, null, 4);
		}
		catch (e)
		{
			data.message = e.formatStack(e);
			//data.message.unshift('')
		}		
		//______________________________________________________________________________________________
		/**
		 *
		this.skip = function skip(dotName, value)
		{
if (true) return true;
		 ...
		 *
		 */
		this.dotNameExists = function(dotName, key)
		{
			var isFound;
			if (key == null)
				isFound = (dotName.substr(1).ov(obj) === undefined);
			else if (dotName === '')
				isFound = ('.' + key).ov(obj);
			else
			{
				var o = dotName.substr(1).ov(obj);
				isFound = (o instanceof Object && key in o);
			}
			return isFound;
		}
		/**
		 *
		 */
		this.evalScript = function evalScript(type, dotName, markerScript, outer)	//, dotName
		{
			var script = (!markerScript.startsWith('$') ? 'obj=' : '') 
					   + markerScript;
															//1st replace all ";#" with dotName [RegExp]
			script = script.replace(/;#/g, ';' + dotName);	//e.g. --> $.1=/xyz/;$.1.lastIndex=3
															//then change number's -- e.g. $.1 --> $[1]
			script = script.replace(/\.(\d+)(?=([.=]|$))/g, '[$1]');	
			script = script.replace(/\$/g, 'obj');			//and finally change $ --> obj
...
//script = markerScript;			
			...
			catch (e) 
			{
				...
				return this.jsonError(msg, markerScript, outer);
			}
			return true;							
		}
		/**
		 *
		 */
		this.invalidMarker = function invalidMarker(dotName, markerScript, outer)
		{
			var msg = 'Invalid JSON escape marker: \n\t[' + dotName + '] is undefined';
			this.jsonError(msg, markerScript, outer)
			return msg;
		}
		/**
		 *	
		 */
		this.jsonError = function jsonError(msg, markerScript, outer)
		{
			var msg = '\nJSON.plus.unescape(): ' + msg;
			if (outer)
			{
				msg += ' at JSON line ' + this.getLineno(outer);
				msg += ':\n\t' + outer.trimPlus('{}').trim()
			}
			else if (markerScript)
				msg += '\n\t' + markerScript;
		 ...
		 *
		 */
		this.getLineno = function getLineno(outer)
		{
			var next = data.json.indexOf(outer, data.offset);		
			if (next == -1)
				return '...'
			data.offset += next + outer.length;
	 ...
	 *
	 */
	function ___linenoFormat(lines)
	{
		var lineno = 0;
	...
	//	var options = EZ.jsonPlusV3.options.common;
		var options = JSON.plusV3.options.common;
		var marker = options.escapeMarker.wrap('@');	
		json = ___cleanStrings(json);
		return (json.includes(marker))			
	}
	//==============================================================================================
	return _init( ___cleanStrings, ___hasMarker, ___jsonUtils,  ___jsonType, ___isCircular, ___linenoFormat );
})();
/*--------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------*/
...
EZ.jsonPlusV3.unescape.test = function JSON_unescape_tests()
{
	...
	var exfn = function exfn(testrun)
	{
		...
		*/
		...
		{
			msg = msg || 'rtnValue.isOk(): ' + rtnValue.isOk();
			//msg = 'message:'.wrap('<em>') + '\n' + msg;
			//msg = msg.replace(/\n\n/g, '\n-\n')
			testrun.appendResults(msg, 'message');
		...
		*/
		...
		/*	
		NOTES:
		2nd agrument actual return value set to returned lists
		//__________________________________________________________________________________________
		*/
		var lists = rtnValue.getLists();					//show returned lists
		if (!lists || Object.keys(lists).length === 0)
	...
	note = 'undefined markers'
	obj = {
		a: {
			"@JSON_escapeMarker@": "undefined"
		}
	}
	EZ.test.options( {note:note} )
	EZ.test.run(obj, '')
...
//________________________________________________________________________________________
EZ.jsonPlusV3.stringify.test = function JSON_stringify_tests()
{	
