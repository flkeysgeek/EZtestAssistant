/***************************************************************************************************
08-15-2016 renamed to EZ.basic.js
***************************************************************************************************/


/*global EZ, EZ$:true, e:true, g, dw, DWfile, unescape */
/*global EZgetPref, EZgetEl, EZgetValue, EZsetValue, EZnone, EZdisplayCaller  */
...
{
	var isEZjsonLoaded = EZ.json.stringify && !EZ.test.running.includes('EZstringify');
...
/*--------------------------------------------------------------------------------------------------
EZ.options(options, defaultKey, defaults)

ARGUMENTS:
	options			(Object) 	caller options as key/values
					(String) 	specifies value of defaultKey if defaultKey is String -or-
							 	if defaultKey =true, options parsed for key/value options
				
	defaultOptions	(Object) 	base options returned -- specified option overriden
					(Boolean)	true to parse key/value pairs from options if String
					(String)	option key if options is not an Object

	defaults		(optional)	currently Object of keys containing list types of options
								to used for options value when not an Object
								pseudo type "element" used when options is html element
...
REFEERENCE:
TODO:
	return options as EZoption type
	//var options = EZ.is(value, EZ.options) && value.keep ? value : new EZ.options(); 

	var options(url, arguments);
EZ.defaultOptions.beep

--------------------------------------------------------------------------------------------------*/
EZ.options = function EZoptions(optValue, defaultOptions, defaults)
{
	if (this instanceof arguments.callee) 		//if called as constructor return EZoptions() class
	{
		/*
		this.toString = function toString() 
		{										
			return EZ.stringify(this, '*all')
		}
		this.includes = function includes(key)	//backward compatibility simulated prior test for options 
		{										//Array or String
	...
	/**
	 *	default validation function used when defaultOptions is native Object
...
EZ.options.get = function EZoptions_get(nameKey, defaultValue)
{
	...
	if (!key)
	{
		...
		return EZ[name].options = allOptions;
	}
...
EZ.get = function EZget(el, defaultValue)
{
   var rtnValue = (window.EZgetValue)
	...
	EZ.set.tags = EZ.tags;
	EZ.set.el = EZ.tags instanceof Object ? EZ.tags[0] : EZ.none();
	return rtnValue;
}
//__________________________________________________________________________________
/**
...
EZ.hide = function EZhide(tags, isTrue)
{
	...
	tags.remove().forEach(function(el)
	{
		...
		if (EZ.popup.remove) EZ.popup.remove(el);
		...
		else if (EZ.getStyle(el, 'display') != 'none')
		{
			...
			if (display)						//save prior if not yet saved
				'.EZfield.displayOrig='.ov(el,display);
			
			el.style.display = 'none';			//set display:none
			//el.displayOrig = EZ.getStyle(el, 'display');
		}
	});
	if (EZ.show.paddingTopOrig !== undefined)
	{
		var el = EZ(['popupPad', 'body'])[0];		
		el.style.paddingTop = EZ.show.paddingTopOrig + 'px';
		delete EZ.show.paddingTopOrig;
	}
}
/*-----------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------*/
EZ.show = function EZshow(tags, isTrue, isAll)
{
	if (EZ.isLegacy('EZshowHide'))
	{
	...
	var _show = function(item)
	{
		...
		item.why.remove('offsets');
		if (item.why.length === 0) return;
		
		if (el.style.visibility == 'hidden' || EZ.hasClass(el, 'invisible'))
		{
			EZ.addClass(el, 'visible')
		}
		if (el.style.display == 'none' || EZ.getStyle(el, 'display') == 'none')
		{
			var display = '.EZfield.displayOrig'.ov(el);
			if (display)
				el.style.display = display;
			
			else if (el.style.display != 'none' && EZ.hasClass(el, 'hidden'))
				EZ.addClass(el, 'unhide')
			
			else
			{									//get default display style value for tag
				display = EZ.dom.display[el.tagName.toLowerCase()];
	...
	tags = EZ(tags, true);						  //----------------------------\\
	tags.remove().forEach(function(el)			 // for each specified tag . . . \\
	{											//--------------------------------\\
		if (!el.childNodes) return;
		var reasons = el.isHidden();
		if (!reasons)							//only visible is reliable
		...
		reasons.every(function(item) 			//crawl up ancesstor chain
		{ 
			_show(item)
...
EZ.beep = function EZbeep(url)
{
	...
	var url = (options.url == 'ding') ? 'EZ.simulator.domain'.ov('') + 'Shared/EZ/sounds/ding.wav'
							  		  : options.url;
	try
	{
	...
	if (options.log)
	{
		...
		if (stack[0].startsWith('@'))
			stack.shift();
		stack.unshift(options.message || 'EZ.beep()');
		var note = (options.log !== true && options.log != options.message) ? options.log : '';
		var title = (note instanceof Object) ? 'detail: [' + EZ.getType(note) + ']' : '';
		if (note)
			stack.push('')
...
--------------------------------------------------------------------------------------------------*/
EZ.beep.alert = EZ.oops = function EZbeep_alert(msg, obj)
{
	var opts = (typeof(msg) == 'object') ? {message:obj, log: msg || obj}
										 : {message:msg, log: obj || msg}
	opts.alert = true;
	return EZ.beep(opts);
}
/*--------------------------------------------------------------------------------------------------
...
EZ.run = function EZrun(fn, options)
{
	...
	if (options !== false)
		EZ.timer.pending.push(fn);
	
	fn();		
	
	if (options !== false && EZ.timer.pending.includes(fn))
		EZ.timer(fn, options);
}
...
EZ.timer = (function()
{
	function EZtimer(msg, options)
	{
		...
		var log = function(msg, timeStr)
		{
			...
			EZ.timer.log.push(logMsg + '\t' + timeStr)
			void(0); 
		}
		//==============================================================================================
		...
		do
		{
			if (msg === undefined || !EZ.timer.start)
			{
				if (EZ.timer.pending.length)
				{
					log('-'.dup(60));
					log('queued functions')
					log('-'.dup(60));
					while (EZ.timer.pending.length)
						EZ.timer( EZ.timer.pending.pop() );	
					log('-'.dup(60));
				}
				EZ.timer.log = [];

				EZ.timer.start = time;
				log( fill(EZ.getCallerName(msg)), 'reset @ ' + EZ.formatTime());
			}
			EZ.timer.total = time - EZ.timer.start;
			EZ.timer.elapsed = time - EZ.timer.last;
			
			var minMS = 'EZ.timer.minMS'.ov(500);
			if (EZ.timer.elapsed < minMS && options !== true) break;
			...
			if (msg)
			{
				...
				{
					var fn = msg;
					var idx = EZ.timer.pending.indexOf(fn);
					if (idx != -1)
						EZ.timer.pending.splice(idx,1);
		
					msg = (fn.name) ? fill(fn.name)
									: {'anonymous': (fn+'').split('\n')};
				}
				if (EZ.timer.lastMsg == msg) break;
				EZ.timer.lastMsg = msg;
				
				var timeStr = 'total/elapsed: ' 
							+ EZ.formatSeconds(EZ.timer.total) + ' ' 
							+ EZ.formatSeconds(EZ.timer.elapsed);
				log(msg, timeStr);
			}
		}
		while (false)
		return {start: EZ.timer.start, time:time, elapsed:EZ.timer.elapsed, total:EZ.timer.total};
	}
	//______________________________________________________________________________________________
	EZtimer.toString = function()
	{
		return EZ.timer.log.join('\n');
	}
	EZtimer.open = function(projectName)		//use saved options if current
	{
		var opts = JSON.parse( sessionStorage.getItem(projectName + '.EZtimer'));
		if (!opts || new Date(opts.timestamp).getTime() < new Date().getTime())
			return EZ.timer(arguments.callee.caller);
		
		Object.keys(opts).forEach(function(key)
		{
			EZ.timer[key] = opts[key];
		});	
		if (!EZ.timer.console) return;
		EZ.timer.log.forEach(function(log)
		{
			var msg = log.split('\t');
			console.log(msg[0],msg[1]);
		});
	}
	EZtimer.reset = function()
	{
		EZ.timer.log = [];
		EZ.timer.pending = [];
	}
	 ...
	 */
	EZtimer.save = function(projectName)		//save current values for reload
	{
...
/*--------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------*/
EZ.formatSeconds = function EZformatSeconds(ms)
{
	var seconds = (ms / 1000) + '';
...
/*--------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------*/
EZ.range.selectNode = function EZrange_selectNode(el) 
{ 	
...
TODO: not tested
--------------------------------------------------------------------------------------------------*/
EZ.createElement = EZ.createLayer = function EZcreateLayer(id, tag, styles)
{
...
EZ.getStackTrace = function EZgetStackTrace()
{
	...
	try
	{
		stack = EZ.stack().lines.join('\n');
		//undefined.getStackTrace;
	}
	catch(e)
	{
		stack = e.stack;	//EZ.stack not loaded -- no worries -- use error stack
	}
	return stack.formatStack({skipCount: 1});
}
/*-----------------------------------------------------------------------------------
...
if value is element, get value  of element.
--------------------------------------------------------------------------------------------------*/
...
EZ.clone = function EZclone(obj, depth)
{
	...
	{	
		...
		try
		{
			var isCircular = EZ.isObjectCircular(obj);
			if (isCircular)
			{
				msg = "EZ.clone() failed -- Circular Object";
				EZ.oops(msg);
				return {"_message": msg};
			}
			var	json = EZ.stringify(obj,'* *circular *keys');	//TODO: circular issues??
			//var json = JSON.stringify(clone, null, 4);	
			try
			{
				eval('clone=' + json);
				if (!EZ.isEqual(clone, obj, true))
				{
					EZ.oops( 'EZ.clone() failed -- not equal',  {details:EZ.equals.formattedLog} );
				}
			}
			catch (e)
			{
				EZ.oops( 'EZ.clone() failed -- json invalid', {details:EZ.parse(json)} );
			}
			return clone;	
		}
		catch (e)
		{
			return EZ.techSupport(e, arguments);
			/*
...
---------------------------------------------------------------------------------------------*/
EZ.compare = function EZcompare(oldValue, newValue)
{
	oldValue = oldValue || EZ.compare.oldValue || 'NA';
	newValue = newValue || EZ.compare.newValue || 'NA';
	
	oldValue = EZ.compare.oldValue = typeof(oldValue) == 'string' ? oldValue 
								 : EZ.stringify(oldValue, '*');
	newValue = EZ.compare.newValue = typeof(newValue) == 'string' ? newValue 
								 : EZ.stringify(newValue, '*');

	var folder = EZ.constant.configPath + 'Shared/EZ/';
	DWfile.write(folder + 'compare.new.txt', newValue);
	DWfile.write(folder + 'compare.old.txt', oldValue);
...
{	
	...
	arg = 'input type=text value=160 size=5 id=maxHeightTestRows'
			+ " onchange=EZ.copyValue('maxHeightTestRows', this.value)"
...
EZ.getOffsets = function EZgetOffsets(el, browserOffsets)
{
	...
	var offsets = [offsetLeft, offsetTop];				//legacy
