/*global 
EZgetPref, EZgetEl, EZgetValue, EZsetValue, EZnone, EZdisplayCaller,

EZ:true, EZ$:true, EZ_:true, e:true, g, dw, DWfile, unescape,
...
EZ.isEqual = (function _____EZequals_____()
{
	...
	{										
		...
		if (options.track)							
			EZ.track();
		//EZ.test.setTestValue('options', options);

		var rtnValue = new EZ.returnValue(this,options);			//init rtnValue Object
		...
		function ___isEqualObjects(x, y, depth, isExcluded)
		{
			...
			do
			{
				...
				{													
					...
					else if ((EZ.isNative(x) || EZ.isNative(y)) && x !== y)
						loggedError = _equalsLog();
...
//		console.log('triggered events', evtList, outerHTML)
	...
	{
		if (!el || !eventName || typeof(eventName) != 'string')
			return EZ.oops('invalid arguments: ', arguments);
	...
	function _triggerAllEvents(el)
	{
		...
		return (results) ? results[1]	//return 1st onload... event if foune
...
//			audio.volume = options.volume || 1;
	...
	{
		var title = '';
		var stack = EZ.getStackTrace(EZ.beep);
