...
(function(ez)
{
	...
	**/
	...
	EZ.defaultFormatOptions = defaultFormatOptions;
	/*--------------------------------------------------------------------------------------------------
...
//__________________________________________________________________________________________________
/**
 *	most functionality of full EZ.toArray() -- does not supports delimiter =false, or Object
 *	TODO: not well tested BUT VERY LIMITED use
**/										
	if (delimiter === true) 
		delimiter = ','
	
	if (typeof(delimiter) == 'string')
		delimiter = RegExp('\\s*?[' + delimiter + ']\\s*');
	
	if ( !(delimiter instanceof RegExp) )
		delimiter = '';
	
	var a = (arg === '' || arg == null || typeof(arg) == 'function') ? []
		  : (arg instanceof Array) ? arg
		  : (arg instanceof Object && 'length' in arg) ? [].slice.call(arg)
		  : (typeof(arg) == 'string') ? []
...
//__________________________________________________________________________________________________
EZ.getCaller = function _____EZgetCaller(caller, exclude) {
//__________________________________________________________________________________________________
/**
 *	Error.prototype.stackTrace() does all the work -- just identify ourself as "this"
**/										
	"use strict";
	EZ.getCaller.format = EZ.getCaller.format || 'callerName';
	return Error.prototype.stackTrace.call(EZ.getCaller, caller, exclude);	
}
/*--------------------------------------------------------------------------------------------------
EZ.getCallerName([fn])
...
//__________________________________________________________________________________________________
EZ.dotName = function EZdotName_boot(key) {
/*{-------------------------------------------------------------------------------------------------
simple dotName until full version loaded -- returns Array() with overridden toString()
non-functional exclude related functions to avoid js error if called.
------------------------------------------------------------------------------------------------}*/
	var dotName = key ? [key] : [];
	dotName.toString = function(key)
	{
	...
	dotName.isExcluded = function() { return false };
	dotName.getSkipList = function() { return [] };
	return dotName;
}
//__________________________________________________________________________________________________
EZ.log = function EZlog_boot() {
/*{-------------------------------------------------------------------------------------------------
save log messages until EZ.log pseudo class loaded and initialized
------------------------------------------------------------------------------------------------}*/
	"use strict";
	...
	var name = this ? this.name || this.toString() 
			 : (caller.name || '-na-');
	EZ.log.queue[name] = EZ.log.queue[name] || [];

	var args = [].slice.call(arguments);
	args.caller = caller;					//verbose but safe
	EZ.log.queue[name].push(args);
}
//__________________________________________________________________________________________________
	/**
 *	TODO: pondering... minimal w/o options ??
	**/
EZ.dotName = function EZdotName_minimal()
			{												
};
//__________________________________________________________________________________________________
...
EZ.date = function EZdate_boot() { return new Date([].slice.call(arguments)) }
//__________________________________________________________________________________________________
...
{	//COPIED BUT NOT REMOVED FROM: EZbasic_pruned.js
/*---------------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------------------------
	...
	**/
	...
	___ = function EZcommon(caller, options, data, callback)							
	{
		...
		__.setupOptions.call(ctx, options);					//...options for class instance
		
		__.setupData.call(ctx, caller, data);				//class or instance initial data
	 ...
	 *
		...
		var fn;
		for (fn in ___.DATA)					//copy EZ.common.DATA to EZ.common
			if (!___[fn])
				___[fn] = ___.DATA[fn];

		for (fn in ___.DATA)					//copy caller.DATA to prototype
			if (!caller.prototype[fn])
				caller.prototype[fn] = ___.DATA[fn];

		for (fn in caller)						//copy caller.___ to prototype ???
			if (!caller.prototype[fn])
				caller.prototype[fn] = caller[fn];
			
		caller["~cid"] = cid;
		caller["~class"] = caller.name;
		caller["~fnList"] =	_getFunctions(caller.prototype);		
		//caller["~nextseq"] = 0;
		//=============
		return caller;
		//=============
	...
	//______________________________________________________________________________________________
	bindFunctions: function(caller, data) {					
	/*{
	 *	bind functions to new Object -- no currently used
...
//__________________________________________________________________________________________________
EZ.logNew = (function _____EZ_LOG_NEW_____() {
//__________________________________________________________________________________________________
/*{
EZ.groupName(o, options)
...
______________________________________________________________________________________________}**/										
	...
	//______________________________________________________________________________________________
	/**
	 *	EZlogItem constructor -- name or type e.g. EZ.log.call('myType', message)
	**/
	function EZlogItem(name, message, detail, key)	
	{
		if (arguments.length == 3 && !(detail instanceof Object))
		{
			key = detail;
			detail = '';
		}
		...
		this.detail = detail || ''
		this.stack = EZ.getStackTrace(EZ.log);
	...
	//==============================================================================================
	/**
	 *	EZlog constructor -- calls add() if not called as cnstructor
	**/
	(function _____EZlog_constructor_____() {
	//==============================================================================================
	___ = function EZlog()							
	{
		if (!EZ.log) 
			return this;						//called as script to create static EZ.log() class
		
		if (this instanceof arguments.callee)
		{
			data = this._data = defaultData()
	...
	/**
	 *	
	___.prototype.toString = function ___toString()
	{
	...
	**/
	 ...
	 */
	___.prototype.add = function ___add(message, detail, key)
	{		
		if (!message || !message.length) return;
		
		var name = EZ.getType(this) == 'String' ? this + ''
				 : (this != window && this != window.EZ) ? this.name
				 : arguments.callee.caller == ___ ? EZ.getCallerName(___)
				 : EZ.getCallerName(true);
					   
		var logItem = new EZlogItem(name, message, detail, key);
		
		var log = _get(name, logItem.key);
		log.push(logItem);
	 ...
	 *	
	 */
	___.prototype.status = function ___status(el)
	{
		var note = '';
		var status = {
	 ...
	 *	Activate log
	 */
	___.prototype.setActive = function ___setActive(showPending)
	{
	 ...
	 */
	___.prototype.isActive = function ___isActive(name, type)
	{
		if (!options.active)
			return false;
		if (!name)
	...
	**/
	...
	//==============================================================================================
	function _____STATIC_functions_____(){}	// {	non log specific functions
	//______________________________________________________________________________________________
	/**
	 *
	**/
	...
	**/
	...
	//______________________________________________________________________________________________
	[_____STATIC_functions_____]	//}	end of static functions
	//______________________________________________________________________________________________
	/**
	**/
	function _init()
	{
		...
		var callback = function() { ___.displayPending() };
		window.addEventListener('load', callback, false);
		return ___
	}
	[_init]
	//==============================================================================================
	e = EZ.common(___, defaultOptions, defaultData, ___.displayPending);
	options = ___._options
	/*
	var _,									//shortcut references to shared EZ.common() function
		_getTag = _.getTag,
		_setTag = _.setTag,
		_formatValue = _.formatValue;
		_sync = _.sync;
	[_sync]
	*/
	return ___
