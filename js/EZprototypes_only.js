...
if (EZ.createPrototype(function formatEZprototypeArray(options)
{
	...
	var args,
		...
		//itemSizesHTML = [],
		lines = this,
		_saveItem = function(text, items)
		{
			var size = text.length;
			if (text.includes('>'))
			{									//esc < or > inside quotes
				var escText = text.replace(/(["'])[^\1]*?\1/g, function(all)
				{ 
					return all.replace(/</g, '###').replace(/>/g, '@@@');
					//all.replace(/(<|&lt;)/g, '###').replace(/(>|&gt;)/g, '@@@');
				});
												//unescape &lt; or &gt;
				//escText = escText.replace(/&lt;/g, '<').replace(/&gt;/g, '>');
												
												//remove html tags
				var textOnly = escText.replace(/(<[^>]*?>\s*([^<]*)*)/g, function()
				{
					return arguments[2] || '';
				});
				
				var pad = textOnly.match(/(\{pad\})/);
				if (!pad)
				{								//append pad control if omitted
					text += '{pad}'
					//textOnly += '{pad}'
				}
	...
	for (var i=0; i<lines.length; i++)
	{
		...
		else
		{
			...
			for (var j=0; j<args.length; j++)
			{
				...
				var size = _saveItem(item, items);
				itemSizes[j] = Math.max(size + 1, itemSizes[j] || 0);
				//itemSizes[j] = Math.max(item.length + 1, itemSizes[j] || 0);
			}
			lineItems.push(items);
		}
	}
	var format = '';						//create format with sizes from tabs
	for (var j=0; j<itemSizes.length; j++)	//e.g. {0:5}{1:10}{2:50}...
	{
	...
	for (var i=0; i<lineItems.length; i++)
	{
		...
		{									//adjust format size if items with html	
			var fmt = format.replace(/\{(\d+):(\d+)\}/g, function(all, idx, size)	
			{								
				[format]
				var value = args[idx] = args[idx] || '';
				if (!value)
					return all;

				var results = value.match(/^###(\d+)###([\s\S]*)/);
				if (!results) 
					return all;
											//arg values of form: ###htmlSize### if html
				var textSize = Number(results[1]);
				var value = results[2];
				
				var pad = size - textSize + 6;
				args[idx] = value.replace(/\{pad\}/, (pad > 0 ? ' '.dup(pad) : '')) + ' ';
				return '{' + idx + ':' + args[idx].length + '}';
			});
			var text = fmt.format(args);
			//text = text.format();
			//if (text.clip) text = text.clip();
			formattedLines.push(text.trim());
		}
	}
...
Array.prototype.format.test = function()
{
	...
	//______________________________________________________________________________
	ctx = [
		"at _____EZgetCaller\t[getCaller]\t ~~406",
		"at EZlog_boot\t[<i>EZlog_boot</i>]\t ~~788",
		"...\t \t ~~791"
	]
	EZ.test.run(ctx)
	...
	ctx = [
		"at _____EZgetCaller\t [getCaller]\t ~~406.<a href=\"stackLink:EZboot.js:406:36\">EZboot.js</a>",
		"at EZlog_boot\t [<i>EZlog_boot</i>]\t ~~788.<a href=\"stackLink:EZboot.js:788:19\">EZboot.js</a>",
		"...\t \t ~~791.<a href=\"stackLink:EZboot.js:791:4\">EZboot.js</a>"
	]
	EZ.test.run(ctx)

	ctx = [
		"at _____EZgetCaller\t [getCaller]\t ~~406.<a href=\"stackLink/EZboot.js:406:36\">EZboot.js</a>",
		"at EZdataFile._getInfo\t [<i>_getInfo</i>]\t ~~680.<a href=\"stackLink/EZdev.js:680:16\">EZdev.js</a>"
	]
	EZ.test.run(ctx)
...
//__________________________________________________________________________________________________
(function _____Error_stackTrace____() {
/**{
returns EZstack Object created from Error.stack which includes caller function name, formatted
stack including lineno and filename.

EZstack.toString() returns stack (in default or specified format) -OR- EZstack property e.g.
	callerName, dotName, filename, lineno
	
If "this" is not Error Object, stack Array or String, a new stack is created via new Error():
	-stack calls THRU Error.toString() are removed including the new Error() default message
	-when "this" is (String), it replaces new Error() message

ARGUMENTS:		(optional)
	format		(String) one of the following formats:
					stack (default)	reformatted stack prefixed with message 
					stackTrace		same as stack w/o message
					console 		stackTrace with url appended
					unfiltered		stack reformatted but no calls excluded
					original		original unfiltered unformatted
					
					html			stackTrace with filename link: console.log(url)
									unfiltered stack appended inside <details> tag
				 
				 OR properties	
					callerName/name, dotName, objName, allNames,
					message, filename, lineno, url
					
	skip		(Number) specifies number stack calls skipped from top of stack
				(Array or delimited String) function names skipped THRU at top of stack 
				
				INCLUDE "-" in list of names to skip names NOT listed
				
				FIRST filter name is ONLY used to trim top of stack THRU the name if found
				if prefixed with "+", stack is trimmed THRU the LAST occurance of the name
				if prefixed with "-", stack is trimmed TO the FIRST occurance of the name
				
				All but 1st filter name(s) are filtered anywhere found 
				(list the 1st name twice to use as filter anywhere in the stack)
						 
	options		Object or delimited String containg one or more of below properties:
					format, skip, -
				
RETURNS:
	EZstack Object containing pretty formatted filtered stack and other variations and following
	properties listed above for top call line after filters applied.
...
________________________________________________________________________________________________
	...
	Error.prototype.stackTrace = function stackTrace_EZ(format, skipList, options) 
	{
		...
		//	args = [].slice.call(arguments),
			...
			stackTraceHTML = [],				//html with <a href="{url:lineno:col}">
			stackUnfiltered = [],
			
			skipCount = 0,
												//not accurate anymore
			stackNames = [],					//primary name for each stackLine
			skippedNames = [],
		...
		//}
		//==============================================================================================
		var _MAIN = function()					//code on top as debugger convenience
		{
			try
			{
				options = _processArguments.apply(this, [].slice.call(arguments));
			...
			catch (e)
			{
				...
				stackTrace = stackTraceHTML = stackUnfiltered = stack.split('\n');
			}
			//========================
			return new EZstack();
			//========================
		...
		**/
		...
		{	
			...
			{											
				var filename = '', asName = '', prefix = '',
					anonymous = false, native = false;	
				...
				var merged = '';
				if (!fn.name && !line)				//use prior line url and lineno
				{									
					stackLines.pop();
					stackTrace.pop();
					stackTraceHTML.pop();
					merged = stackUnfiltered.pop();
				}
				else if (name)
				{
					fn = {									
				...
				else if (pre)							//ignore pre if native and pre.name is not
				{										//e.g. Function.myfunc	drop Function
					pre = pre.slice(0,-1);				//	   Array.forEach	keep Array
					if (!native
					&& (window[pre] || '').toString().includes('[native'))
					{									//pre is native
						if ((window[pre][name] || '').toString().includes('[native'))
							native = true;				//...and so is pre.name -- keep
							
						else if ((window[pre].prototype && name in window[pre].prototype))
							native = true;				//...also HTMLElement.onchange
							
						if (!native)					//don't keep if not all native
							pre = '';
					}
					if (pre) pre += '.';
				}
				if (name)
				{
					...
					{
						fn.names.push(fn.name);
						if (fn.asName && fn.name != fn.asName) 
							fn.names.push(fn.asName);
					}
				}			
				stackNames.push(fn.names[0] || '-na-')
				stackLines.push(fn);
															
				asName = asName ? asName.wrap('[]')
				//	   : (fn.name) ? fn.name.wrap('[]')
					   : (fn.name) ? fn.name.wrap('<i>').wrap('[]')
					   : '';
				
				msg = (fn.dotName ? 'at ' + fn.prefix + fn.dotName : '...')
					+ '\t ' + (asName)
					+ '\t ' + (fn.lineno ? fn.lineno.pad(-5,'~') + '.' : '~'.dup(6))
				
				stackTrace.push(msg.replace(/<\/?i>/g, '') + fn.filename);
				
				/*
					<a href="stackLink:http://localhost:8080/.../js/EZdev.js:517:8">EZdev.js</a>
					_stackLink() calls console.log() onClick() -- EZtooltip.css has stackLink
					for <a...> title and brief flashing message when clicked
				*/
				var link = '<a href="stackLink:' + fn.url + fn.line + '">' + fn.filename + '</a>';
				stackTraceHTML.push(msg + link);			
				
				msg = (!fn.dotName) ? msg.replace(/^at/, '...') 
					: (merged) ? 'at ' + fn.prefix + fn.dotName + '\t \t~~...'			
					: msg;		
				if (merged)							//keep both lines																
				{									//but clear link on 2nd line
					link = '';						//because there is no url/lineno
					msg = merged + '\n' + msg;							
				}
				stackUnfiltered.push(msg + link);	//copy of html stack not later filterd
				
				//stackTraceConsole.push(msg + ' \t' + fn.url + fn.line);	//redunsant ??
		...
		**/
		...
		{										//determine if any calls on stack skipped \\
			var	skipToIdx = 0,					//-----------------------------------------\\
				//skipAnonymous = false,			//anonymous not skipped at top of stack
				skipTo = ' ',					//space to skip listed name at top
												// "-" skip not listed name at top
												// "+" skip to last listed anywhere in stack
				skipThru = true,				//=false skip up to (* in skip list)
				
				skipRegEx = options.skipRegEx = [],
				skipNames = options.skipNames = [];
			
			//stackInfo.topStackName = stackNames[0];
			skipList = (options.skip instanceof Array) ? options.skip.slice() 
					 : (options.skip === '' || typeof(options.skip) == 'string') ? []
					 : options.skip.split(/\s*[ ,]\s*/);	//keeps empty item
			
			skipList.forEach(function(fn)	//for each listed name
			{
				if (fn instanceof RegExp)
					skipRegEx.push(fn);
				else if (typeof(fn) == 'function')
					skipNames.push(fn.name);
				else if (!isNaN(fn))
					skipToIdx = Number(fn);
					...
					_log('ignored invalid skip function name: ' + fn);
			});
			
			if (!skipToIdx && callerName)	// && stackCreated)
				skipToIdx = 1;
													//------------------------\\
			if (skipToIdx)							//skip - or + Number . . . \\
			{										//--------------------------\\
				if (Math.abs(skipToIdx) >= stackLines.length)
					_log('ignored skip count[' + skipToIdx + '] -- exceeds stack size[' + stackLines.length + ']');
				else
				{
					var count = skipToIdx;	// + 1;
					skipCount += count;
					
					stackLines.splice(0, count);
					stackTrace.splice(0, count);
					stackTraceHTML.splice(0, count);
					
					var names = stackNames.splice(0, count);
					[].push.apply(skippedNames, names);
				}
			}
														
			var isSkip = true;							//--------------------------\\
			if (skipNames.length || skipRegEx.length)	//skip using listed names . . . \\
			{											//----------------------------\\
				skipToIdx = -1;			
				...
				{										
					...
					if (isSkip)
					{
						if (stackItem.anonymous || stackItem.native)
							continue;
					}
					var skipName = '';					//ckeck skip lists
					var isFound = stackItem.names.some(function(name)
					{									//check if any name is skipped
						if (!skipNames.includes(name)) return;
						return skipName = name;
					});
					isFound = isFound || skipRegEx.some(function(regex)
					{									//check if any regex with any name
						return stackItem.names.some(function(name)
						{
							...
							return skipName = name;
														...
														//---------------------------------\\
					if ((isFound && skipTo == '-')		//what does it mean if found ot not \\
					|| (!isFound && skipTo == ' '))		//-----------------------------------\\
					{		
						skipToIdx = idx;				//done if skipping to and found	-OR-
						if (skipTo == '-' && skipThru)
							skipToIdx++;
						break;							//skipping listed and not found
					}
					if (isFound)						//matched name in list. . .
					{
						isSkip = false;
						if (skipTo == '+')				//if skipping all, start canidate
							skipToIdx = idx;			
						else
							isSkip = true;				//skip anony and keep looking		
					}
					else if (skipTo == '-')				//not found and skipping to
						isSkip = true;					//skip anony and keep looking		
					else
						log('confused...');
				...
				if (skipToIdx == -1)
					_log('ignored skip to name(s) -- no matches');
				
				else if (skipThru)						//bump skipToIdx if skipping past
				{
					//skipToIdx++;
					for (idx = skipToIdx; idx < stackLines.length; idx++)	
					{										
						var stackItem = stackLines[idx];
						if (!stackItem.anonymous && !stackItem.native)
							break;
				...
				if (skipToIdx >= stackLines.length)
					_log('ignored skip to name(s) -- skips entire stack');
				
				else if (skipToIdx > 0)
				{
					skipCount += skipToIdx;
					stackNames = stackNames.slice(skipToIdx);
					stackLines = stackLines.slice(skipToIdx);
					stackTrace = stackTrace.slice(skipToIdx);
					stackTraceHTML = stackTraceHTML.slice(skipToIdx);
				}
			}
		}
		//______________________________________________________________________________________________
		/**
		 *	create EZstack Object
		**/
		function EZstack()				
		{
			stackLines = stackLines || [defaultStackLine()];
			var obj = stackLines[0];
			obj["~name"] = 'EZstack';
			obj.stackCalls = stackLines;			//properties for all stack calls
													//...stackLines merged
			...
			log = log.join('\n').trim();
			if (log)
				log = 'Error.EZstack()...' + log;
			obj.log = log;
			
			obj.counts = {
				message: message.length,
				kept: stackTrace.length, 
				original: stackOriginal.split('\n').length,
				total: 0, skipped: 0, unfiltered: 0		//set below
			};
			...
			stackTrace = stackTrace.format().join('\n').replace(/~/g, ' ').trim(); 
			obj.stack = message + stackTrace;
			obj.stackTrace = stackTrace;		
														//------------------\\
														//format html stacks \\
														//--------------------\\
			var idx = stackUnfiltered.length - obj.counts.kept;	//obj.stackIdx
			if (stackUnfiltered[idx])
				stackUnfiltered[idx] = 					//make 1st un-filtered stack item bold
				stackUnfiltered[idx].replace(/(^at .*?)(?=\t)/, '$1'.bold());
														
														//get skipped from un-filtered
			stack = stackUnfiltered.slice(0, stackUnfiltered.length - obj.counts.kept);
			stack = stack.join('\n').split('\n')		//explode merged lines
			obj.counts.skipped = stack.length;		
														
			stack = stack.concat(stackTraceHTML);		//combine skipped and kept
EZ.lastStack = stack;			
			stack = stack.format();						//...format together
			obj.counts.total = stack.length;
														//split back into skipped and kept
			obj.stackSkipped = stack.splice(0, obj.counts.skipped);
			obj.stackSkipped = obj.stackSkipped.join('\n').replace(/~/g, ' ').trim(); 
			
			obj.htmlLines = stack.slice();
			stack[0] = stack[0].replace(/(^at .*?)(?= )/, '$1'.bold());
			obj.stackTraceHTML = stack.join('\n').replace(/~/g, ' ').trim(); 

														//create final html			
			var summary = '(' + obj.counts.total + ' lines)';
			var details = '';
			if (obj.counts.total > obj.counts.kept)
			{
				summary = '('	+ obj.counts.skipped + ' of ' + obj.counts.total + ' lines filtered)';
				details = '<div class="filtered pre">' + obj.stackSkipped + '</div>';
			}
			var html = '<details>'
					 +	  '<summary>stack ' + summary + '</summary>'
					 +   details
					 + '</details>'
														//explode merged lines for format / count
			stackUnfiltered = stackUnfiltered.join('\n').split('\n');
			obj.counts.unfiltered = stackUnfiltered.length;
			
			obj.stackUnfiltered = message				//un-filtered stack
								+ stackUnfiltered.format().join('\n').replace(/~/g, ' ').trim();
														
			obj.html = message					//html stack: message [-]skip [+]kept 
					 + html.wrap('<div>')
					 + '<div class="pre">'
					 + 		obj.stackTraceHTML.replace(/^\.\.\./gm, '   ')
					 + '</div>'
														//htmlPlus stack: html plus unfiltered
			obj.htmlPlus = obj.html						//...only diff is kept lines not merged
						 + '<details style="margin:2px 0 0px 0">'
						 + 		'<summary>un-filtered stack ('
						 + 			(obj.counts.message + obj.counts.unfiltered) 
						 + 			' lines)'
						 +		'</summary>'
						 + 		obj.stackUnfiltered.wrap('<div class="pre">')
						 + '</details>'
			
			obj.debug = obj.html						//debug stack: html stack plus original
						+ '<details style="margin:0px 0 0px 0">'
						+ 		'<summary>original stack ('
						+ 			obj.counts.original +' lines)'
						+		'</summary>'
						+ 		obj.original.wrap('<div class="pre hidden">')
						+ '</details>';			
			//______________________________________________________________________________
			 ...
			 *	any other:			returns coresponding property if found else stack
			**/
			obj.toString = function(format)
			{
				format = (!format || typeof(format) == 'object') ? this.options.format || 'stack'
				...
				return this[format] || this[format.toLowerCase()] || this.stack;
			}
			//______________________________________________________________________________
			if (window.g) g.st = obj;
			if (log)
				console.log(log, {stack: {formatted:obj.console, original:obj.original}});
			return obj;
		}
		//==============================================================================================
		var _processArguments = function()
		{
			var format, skip, options;
			if (this.caller)						//called by interface e.g. EZ.getStack()
			{										//i.e. NOT Error.toString() prototype call
				error = this.error;
				callerName = (typeof(this.caller) == 'function') ? this.caller.name 
																 : this.caller;
				format = this.defaultFormat || this.format;
			}
			var EZoptions = (EZ && EZ.options) || Boolean;
			...
			args.forEach(function(arg)
			{
				...
				{
					if (arg["~name"] == "EZstack" || arg instanceof EZstack)
						error = arg;
					
					else if (arg instanceof Array)
						skip = arg;
					
					else if (arg["~name"] == "EZoptions" || arg instanceof EZoptions || args.length == 1)
						options = arg;
				}
				else if (typeof(arg) == 'string')
				{
					if (arg.startsWith('?'))
						options = EZ.options.call(arg)
			...
			if (error instanceof EZstack
			|| (error instanceof Object && error["~name"] == 'EZstack'))
			{											//TODO: check for diff options
				return error;							//existing EZstack Object
			}
			/*
			if (format instanceof Array || /[ ,]/.test(format))
			{
				skip = format;
				format = ''; 
			}
			...
			*/
			...
			EZ && EZ.sync && EZ.sync(options, defaultOptions());
			options.format = format || options.format || '';
			options.skip = skip || options.skip || options.exclude || '';
			/*
			...
			options = (options instanceof Array) ? {skip: options}
					: (options instanceof Object) ? options
					: (options && window.EZ && EZ.options) ? EZ.options.call(options) 
					: {};	
															
			*/
			// {error:this, caller:EZ.getCaller, defaultFormat:'callerName'};
			
			if (error instanceof Array || error instanceof String) 
				error = {stack: error}; 
			
			stack = '';
			if (error instanceof Object && typeof(error) != 'function')
			{
				stack = (error.stack instanceof Array) ? error.stack.join('\n') 
					  : (typeof(error.stack) == 'string') ? error.stack.split('\n') 
					  : '';
				
				message = options.message || (error.message instanceof Array) ? error.message 
						: (typeof(error.message) == 'string') ? error.message.split('\n') 
						: [];
			}
			else if (options.message)
			{
				message = (options.message instanceof Array) ? options.message
						: (typeof(options.message) == 'string') ? options.message.split('\n')
						: ['unrecognized message:'].concat((options.message+'').split('\n'));
			}
			...
			{											
				...
				stack = stack.slice(4);					//us i.e. EZstack() from stack
				if (!callerName)
		...
		//==============================================================================================
		if (!_stackLinkActivated && document.body)
		{												//TODO: called too often on body
			_stackLinkActivated = true;
			document.body.addEventListener('click', _stackLink, true);
		}
		return _MAIN.apply(this, [].slice.call(arguments));
	}
	//__________________________________________________________________________________________________
	var _stackLinkActivated;
	var _stackLink = function(evt)
	{ 	
		var el = evt.toElement || {};
		if (el.href && el.href.startsWith('stack'))
		{
			console.log(el.href.replace(/stackLink:/i,''));
			EZ.addClass(el,'blink=10000')
			EZ.event.cancel(evt, true);
		}
	}
	//..............................................................................................
})()//...end of _____Error_stackTrace____

//__________________________________________________________________________________________________
TypeError.prototype.toString = Error.prototype.toString = function()
{
